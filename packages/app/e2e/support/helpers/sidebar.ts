...
export async function expectWorkspaceRowHoverCleared(
  ...
  const workspaceKey = `${getServerId()}:${workspaceId}`;
  await expect(page.getByTestId(`sidebar-workspace-archive-${workspaceKey}`)).toBeHidden();
  await expect(page.getByTestId("workspace-hover-card")).toHaveCount(0);
}
...
async function openWorkspaceSidebarContextMenu(page: Page, workspaceId: string) {
...
// The workspace context menu items carry no web ARIA role, so the sidebar suite addresses
// them by the stable test ids the app assigns per workspace — the same convention the
// rename flow uses.
export async function clickArchiveWorkspaceMenuItem(
  page: Page,
  workspaceId: string,
): Promise<void> {
  const serverId = await openWorkspaceSidebarContextMenu(page, workspaceId);
