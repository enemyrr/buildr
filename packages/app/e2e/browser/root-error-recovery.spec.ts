...
async function openBrokenStartup(page: Page) {
  ...
  await expect(page.getByText("Buildr ran into a problem.", { exact: true })).toBeVisible();
}
...
async function reloadToPicker(page: Page) {
  ...
  await expect(page.getByText("Buildr ran into a problem.", { exact: true })).toHaveCount(0);
}
