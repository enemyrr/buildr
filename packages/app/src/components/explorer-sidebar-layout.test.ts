...
describe("Explorer sidebar layout", () => {
  it("keeps the sidebar width fixed when the workspace body changes size", () => {
    const narrow = resolveExplorerSidebarDockSizes({ requestedWidth: 480, containerWidth: 1200 });
    const wide = resolveExplorerSidebarDockSizes({ requestedWidth: 480, containerWidth: 1520 });

    expect(narrow[1] * 1200).toBeCloseTo(480);
    expect(wide[1] * 1520).toBeCloseTo(480);
  });

  it("has no fixed maximum while preserving room for the workspace body", () => {
    expect(resolveExplorerSidebarWidth({ requestedWidth: 100, containerWidth: 1200 })).toBe(408);
    ...
    expect(resolveExplorerSidebarWidth({ requestedWidth: 600, containerWidth: 750 })).toBe(408);
  });
});
