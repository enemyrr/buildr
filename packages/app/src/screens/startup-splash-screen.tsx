...
import { PASEO_LOGO_PATH, PASEO_LOGO_VIEWBOX, PaseoLogo } from "@/components/icons/paseo-logo";
...
function WebLogoShimmer({ color }: { color: string }) {
  ...
  const shimmerStyle = useMemo(
    () => ({
      width: LOGO_SIZE,
      height: LOGO_SIZE,
      WebkitMaskImage: `url("data:image/svg+xml,${encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' width='${LOGO_SIZE}' height='${LOGO_SIZE}' viewBox='0 0 ${PASEO_LOGO_VIEWBOX} ${PASEO_LOGO_VIEWBOX}'><path fill='black' d='${PASEO_LOGO_PATH}'/></svg>`)}")`,
