...
export function buildReviewInstructions({ baseRef, prUrl }: ReviewInstructionsContext): string {
...
Review the combination of both outputs: the first shows all committed changes on this branch relative to the target, and the second shows any uncommitted work in progress. Start with \`--stat\` to understand the files that changed, then read specific files as needed.

## Output format

Do not modify any files. Only post ONE finding per unique issue.

Write out a list of issues found, along with the location of the finding. For example:
...
File: src/client/frontends/desktop/core/UserData.ts:10
</example>
`;
}
