...
describe("ForgeService", () => {
  ...
  it("reuses the gh repo view across PR lookups", async () => {
    ...
    const runner: GitHubCommandRunner = async (args, options) => {
      ...
      if (args[0] === "api" && args[1]?.startsWith("repos/")) {
        return { stdout: JSON.stringify([{ number: 42 }]), stderr: "" };
      }
    ...
    expect(calls.filter((call) => call.args[0] === "repo")).toHaveLength(1);
  });
  ...
  it("finds a PR inside the origin fork when gh resolves to the upstream remote", async () => {
    ...
    // The default and origin lookups run in parallel, so answers route by repository.
    const runner: GitHubCommandRunner = async (args) => {
      if (args[0] === "repo" && args[1] === "view") {
        return {
          stdout: JSON.stringify({ owner: { login: "parentOwner" }, name: "repo", parent: null }),
          stderr: "",
        };
      }
      if (args[0] === "pr" && args[1] === "list") {
        const repo = args.includes("--repo") ? args[args.indexOf("--repo") + 1] : null;
        return { stdout: JSON.stringify(repo === "forkOwner/repo" ? [forkPr] : []), stderr: "" };
      }
      if (args[0] === "api" && args[1] === "graphql") {
        return { stdout: currentPullRequestGithubFactsJson(), stderr: "" };
      }
  ...
  it("finds a PR inside a fork that has a parent repository", async () => {
    const forkPr = {
      number: 12,
      url: "https://github.com/forkOwner/parentRepo/pull/12",
      title: "Fork-internal PR",
      state: "OPEN",
      isDraft: false,
      baseRefName: "main",
    ...
    const runner: GitHubCommandRunner = async (args) => {
      ...
      if (args[0] === "pr" && args[1] === "list" && args.includes("forkOwner/parentRepo")) {
        return { stdout: JSON.stringify([forkPr]), stderr: "" };
      }
      if (args[0] === "api" && args[1] === "graphql") {
        return { stdout: currentPullRequestGithubFactsJson(), stderr: "" };
      }
    ...
    const service = createGitHubService({
      runner,
      resolveGhPath: async () => "/usr/bin/gh",
      resolveRepoSlug: async () => "forkOwner/parentRepo",
      now: () => 100,
    });
    ...
    expect(status).toMatchObject({ number: 12, repoOwner: "forkOwner", repoName: "parentRepo" });
  });
