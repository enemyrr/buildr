...
async function resolveCurrentPullRequestView(options: {
  ...
  if (!headRepositoryOwner) {
    ...
    if (parentOwner && parentName) {
      const parentRepo = `${parentOwner}/${parentName}`;
      // A PR opened inside the fork itself lives in origin, so origin is asked alongside the parent.
      const originMatch = findPullRequestInOriginRepo({ ...options, searchedRepo: parentRepo });
      originMatch.catch(() => {});
      const forkMatch = await resolveForkPullRequestView({
        ...options,
        forkOwner,
        repo: parentRepo,
      });
      return forkMatch ?? originMatch;
    }
    headRepositoryOwner = forkOwner;
    searchedRepo = repo?.name ? `${forkOwner}/${repo.name}` : null;
  }
